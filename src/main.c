...
static const char *font_path()
{
    ...
    snprintf(buf,sizeof buf,"%sresources/fonts/OpenSans-Regular.ttf", base?base:"" );
    SDL_free(base);
    return buf;
}
...
int main(int argc,char **argv){(void)argc;(void)argv;
    ...
    if(TTF_Init()!=0){SDL_Log("TTF: %s",TTF_GetError());SDL_Quit();return 1;}
    if (!(IMG_Init(IMG_INIT_PNG) & IMG_INIT_PNG)) {SDL_Log("IMG_Init failed: %s", IMG_GetError());SDL_Quit();return 1;}
    ...
    int running=1;while(running){
        ...
        Uint32 dt=SDL_GetTicks()-frame_start; if(dt<target_ms) SDL_Delay(target_ms-dt);
    }
