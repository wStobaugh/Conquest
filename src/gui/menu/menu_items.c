...
void menu_build_placeholder(Menu *m,const char *title){
    menu_clear_buttons(m);
    char txt[64]; snprintf(txt,sizeof txt,"< %s – [Back] >",title);
    menu_add_button(m,txt,"goto_main", m->win_h-100);
}

void menu_build_from_signal(Menu *m,const char *sig){
    if(!strcmp(sig,"goto_main"))          menu_build_main(m);
    else if(!strcmp(sig,"goto_continue")) menu_build_placeholder(m,"Continue");
    else if(!strcmp(sig,"goto_newgame"))  menu_build_placeholder(m,"New Game");
    else if(!strcmp(sig,"goto_settings")) menu_build_placeholder(m,"Settings");
    else if(!strcmp(sig,"goto_credits"))  menu_build_placeholder(m,"Credits");
}
